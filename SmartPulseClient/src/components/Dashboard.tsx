...
function Dashboard() {
  ...
  // Dinamik API çağrıları için yardımcı fonksiyon
  const buildQueryString = (companyId?: number, sourceId?: number): string => {
  ...
  // Verileri çeken fonksiyon (Interceptor'lı api.ts kullanıyor)
  const fetchData = async (companyId?: number, sourceId?: number) => {
    const queryString = buildQueryString(companyId, sourceId);
    
    try {
      // Artık header eklemene gerek yok, api.ts bunu senin için yapıyor!
      const summaryResponse = await api.get(`/Feedback/summary${queryString}`);
      setSummary(summaryResponse.data);
      
      const trendResponse = await api.get(`/Feedback/trend${queryString}`);
      setTrendData(trendResponse.data);
      
      const feedbacksResponse = await api.get('/Feedback');
      setAllFeedbacks(feedbacksResponse.data.slice(-50).reverse());
      
      const sourceResponse = await api.get(`/Feedback/source-distribution${queryString}`);
      setSourceStats(sourceResponse.data);
      
      const keywordsResponse = await api.get(`/Feedback/top-keywords${queryString}`);
      setTopKeywords(keywordsResponse.data);
    } catch (error: any) {
      console.error("Veri çekilirken yetki hatası oluştu:", error);
      // Eğer token süresi dolduysa (401 hatası), kullanıcıyı login'e atabiliriz
  ...
  const handleFilterChange = (filters: { companyId?: number; sourceId?: number }) => {
    setSelectedCompanyId(filters.companyId);
    setSelectedSourceId(filters.sourceId);
    fetchData(filters.companyId, filters.sourceId);
  };

  // Logout fonksiyonu
  const handleLogout = () => {
    // Tarayıcı hafızasındaki anahtarı yok et
    localStorage.removeItem('token');
    // Sayfayı yenileyerek kullanıcıyı otomatik olarak Login'e fırlat
  ...
  return (
    <div style={{ padding: '0', backgroundColor: '#f8fafc', height: '100vh', width: '100vw', boxSizing: 'border-box', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      
      {/* Navbar */}
      <nav style={styles.navbar}>
        <h2 style={{ color: 'white', margin: 0, fontSize: '18px', fontWeight: 'bold' }}>SmartPulse Analiz</h2>
        <button 
      ...
      <div style={{ padding: '15px 30px', display: 'flex', flexDirection: 'column', gap: '15px', flex: 1, overflow: 'auto' }}>
        {/* Filter Bar */}
